...
const UD = 'C:\\Users\\PC-ali\\AppData\\Local\\Temp\\fake-telegram\\edge-t';
const PORT = 1458;
...
(async () => {
  ...
  const dbg = require('child_process').spawn(EDGE, ['--headless=new','--disable-gpu','--no-sandbox','--disable-dev-shm-usage','--user-data-dir='+UD,'--remote-debugging-port=4829','about:blank'], {stdio:'ignore'});
  let list=null;
  for(let i=0;i<30;i++){ try{ list=JSON.parse(await get('http://localhost:4829/json/list')); if(Array.isArray(list)&&list.length)break; }catch(e){} await wait(400); }
  ...
  async function geo(w,h,touch,label){
    await send('Emulation.setDeviceMetricsOverride',{width:w,height:h,mobile:touch,hasTouch:touch,deviceScaleFactor:2});
    ...
    await wait(500);
    const g = await send('Runtime.evaluate',{expression:"(function(){ function f(s){ var e=document.querySelector(s); if(!e) return s+' NONE'; var r=e.getBoundingClientRect(); return s+' L='+Math.round(r.left)+' W='+Math.round(r.width); } var coarse=window.matchMedia('(pointer: coarse)').matches; return 'VW='+window.innerWidth+' touch(coarse)='+coarse+' | '+[f('#chat-list-column'),f('#conversation'),f('#nav-sidebar')].join(' | '); })()"});
    console.log(label+': '+g.result.result.value);
  }
  await geo(1280,900,true,'1280 TOUCH (was tablet)');
  await geo(1280,900,false,'1280 NO-TOUCH (desktop)');
  await geo(412,915,true,'412 portrait touch');
