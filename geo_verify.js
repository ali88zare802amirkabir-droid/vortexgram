...
const UD = 'C:\\Users\\PC-ali\\AppData\\Local\\Temp\\fake-telegram\\edge-v2';
const PORT = 1458;
...
(async () => {
  ...
  const dbg = require('child_process').spawn(EDGE, ['--headless=new','--disable-gpu','--no-sandbox','--disable-dev-shm-usage','--user-data-dir='+UD,'--remote-debugging-port=4826','about:blank'], {stdio:'ignore'});
  let list=null;
  for(let i=0;i<30;i++){ try{ list=JSON.parse(await get('http://localhost:4826/json/list')); if(Array.isArray(list)&&list.length)break; }catch(e){} await wait(400); }
  ...
  async function geo(w,h,label){
    ...
    await wait(500);
    const g = await send('Runtime.evaluate',{expression:"(function(){ function f(s){ var e=document.querySelector(s); if(!e) return s+' NONE'; var r=e.getBoundingClientRect(); return s+' L='+Math.round(r.left)+' W='+Math.round(r.width); } return 'VW='+window.innerWidth+' | '+[f('#chat-list-column'),f('#conversation'),f('#nav-sidebar')].join(' | '); })()"});
    console.log(label+': '+g.result.result.value);
  }
  await geo(800,360,'800 landscape');
  await geo(390,844,'390 portrait');
  await geo(1100,800,'1100 tablet');
  await geo(1300,900,'1300 desktop');
