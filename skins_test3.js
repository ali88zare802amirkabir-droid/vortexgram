...
const UD = 'C:\\Users\\PC-ali\\AppData\\Local\\Temp\\fake-telegram\\edge-skins3';
const PORT = 1458;
...
(async () => {
  ...
  const dbg = require('child_process').spawn(EDGE, ['--headless=new','--disable-gpu','--no-sandbox','--disable-dev-shm-usage','--user-data-dir='+UD,'--remote-debugging-port=4812','about:blank'], {stdio:'ignore'});
  let list=null;
  for(let i=0;i<30;i++){ try{ list=JSON.parse(await get('http://localhost:4812/json/list')); if(Array.isArray(list)&&list.length)break; }catch(e){} await wait(400); }
  ...
  await send('Runtime.evaluate',{expression:"localStorage.setItem('ft_token','"+TOKEN+"'); localStorage.setItem('vx_owned_skins', JSON.stringify(['default','dark-rose','matrix','synth','sunset','forest','light','premium-black'])); location.reload();"}); await wait(2500);
  ...
  // Click 'dark-rose' skin (theme: midnight-rose)
  await send('Runtime.evaluate',{expression:"(function(){var cards=document.querySelectorAll('.skin-card'); for(var i=0;i<cards.length;i++){ if(cards[i].dataset.id==='dark-rose'){ cards[i].querySelector('.skin-act').click(); break; } }})()"});
  await wait(500);
...
setTimeout(()=>{console.log('WATCHDOG TIMEOUT');process.exit(8);}, 50000);