...
const UD = 'C:\\Users\\PC-ali\\AppData\\Local\\Temp\\fake-telegram\\edge-fn';
const PORT = 1458;
...
(async () => {
  ...
  const dbg = require('child_process').spawn(EDGE, ['--headless=new','--disable-gpu','--no-sandbox','--disable-dev-shm-usage','--user-data-dir='+UD,'--remote-debugging-port=4833','about:blank'], {stdio:'ignore'});
  let list=null;
  for(let i=0;i<30;i++){ try{ list=JSON.parse(await get('http://localhost:4833/json/list')); if(Array.isArray(list)&&list.length)break; }catch(e){} await wait(400); }
  ...
  async function load(){
    await send('Emulation.setDeviceMetricsOverride',{width:412,height:915,mobile:true,deviceScaleFactor:2});
  ...
  console.log('Open chat @412:', conv.result.result.value);
  ...
  console.log('After back @412:', back.result.result.value);
  ...
  console.log('Menu tap @412:', nav.result.result.value);
